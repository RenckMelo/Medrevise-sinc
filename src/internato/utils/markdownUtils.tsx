...
export const ClinicalEcgViewer = ({ text }: { text: string }) => {
  ...
  const titleLine = lines.find(l => l.toLowerCase().includes('ecg') || l.toLowerCase().includes('traçado') || l.toLowerCase().includes('esboço') || l.toLowerCase().includes('derivaç')) || 'Eletrocardiograma (ECG) - Traçado Vetorial';
  ...
  const { pathData, beatsCount } = useMemo(() => {
    ...
    while (currentX < canvasWidth) {
      ...
      // QRS Complex
      const qX = currentX + w * 0.35;
      const rX = currentX + w * 0.42;
      const sX = currentX + w * 0.48;

      // ST & T wave
      const stStartX = currentX + w * 0.52;
      const tPeakX = currentX + w * 0.72;
      const tEndX = currentX + w * 0.88;

      // Elevations & Depressions
      const rHeight = hasHighR ? 85 : 75;
      const stLevel = hasStElevation ? -25 : hasStDepression ? 20 : 0;
      ...
      pathStr += ` Q ${tPeakX} ${baselineY + stLevel - 20} ${tEndX} ${baselineY}`;
  ...
  }, [bpm, isTrn, hasStElevation, hasStDepression, hasHighR]);
...
\`\`\``}
      ...
      <div className="bg-stone-50 px-5 py-2.5 border-b border-stone-200 flex flex-wrap gap-2 text-xs font-semibold">
        ...
        {hasStElevation && (
          <span className="bg-rose-100 text-rose-900 px-2.5 py-1 rounded-md border border-rose-300 flex items-center gap-1">
            <ShieldAlert className="w-3.5 h-3.5 text-rose-600" /> Supradesnivelamento de ST
          </span>
        )}
        {isPosteriorMirror && (
      ...
      <div className="p-4 sm:p-5 bg-stone-900 text-emerald-300 font-mono text-xs leading-relaxed overflow-x-auto border-t border-stone-800">
        <div className="text-[10px] uppercase font-bold tracking-widest text-stone-400 mb-2">Transcrição do Achado de ECG:</div>
        <pre className="m-0 font-mono whitespace-pre select-all text-emerald-300 leading-relaxed font-bold">{rawText}</pre>
      </div>
    </div>
  );
};
...
export const markdownComponents: any = {
  ...
  code: ({ inline, className, children, ...props }: any) => {
    ...
    // 6. ECG Waveforms & Vector Drawings
    const isEcgWaveform = (
      ...
      (codeContent.includes('P - QRS') && codeContent.includes('wave'))
